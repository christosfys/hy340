...
%type <stmt> stmt loopstmt break continue stmts block stmt_opt whilestmt forstmt returnstmt ifstmt function //stmtss
...
break: BREAK SEMICOLON{
              ...
              $$= (struct stmt_t *)malloc(sizeof(struct  stmt_t));
                 if(loopcount==0){red();printf("Error:cannot use break outside of a loop in line:%d\n",yylineno);reset();}
               make_stmt($$); 
              ...
              |expr ADD expr {printf("Expr+Expr %d\n",yylineno);  
                                 ...
                                 $$->sym = insert_symbol(symbolTable,1,s,LOCALL,newFunction(),newVariable(),yylineno,current_scope);
                                  emit(add,$1,$3,$$,yylineno,0);
                                   $$->sym->offset= top_localfunc()+1;                                    
                                                  pop_localfunc();
                                                  insert_localfunc( $$->sym->offset);
    ...
    $$->sym->offset= top_localfunc()+1;                                    
            ...
            $$->sym = insert_symbol(symbolTable,1,newtempname(),LOCALL,newFunction(),newVariable(),yylineno,current_scope); 
             $$->sym->offset= top_localfunc()+1;                                    
              ...
              |term{printf("Terminal %d\n",yylineno); $$= $1;}//(expr *)malloc(sizeof(expr));}
              ;
...
term:         LEFT_PARENTHESIS expr RIGHT_PARENTHESIS{ printf("( expression )\n"); $<expr>$=$<expr>2; }
              |MINUS expr %prec UMINUS  { 
                                          ...
                                          if(check_arith($2)==0){                  
                                            ...
                                            $<expr>$->sym = newtemp();
                                             $<expr>$->sym->offset= top_localfunc()+1;                                    
                                                  pop_localfunc();
                                                  insert_localfunc($<expr>$->sym->offset);
...
primary:      lvalue{printf("lvalue\n");
              ...
              |LEFT_PARENTHESIS funcdef RIGHT_PARENTHESIS{printf("(Function definition)\n");$$=newexpr(programmfunc_e);
                                                    $$->sym = $2;}//$$= expr *newnode = (expr *)malloc(sizeof(expr));}
...
objectdef:    LEFT_BRACKET elist RIGHT_BRACKET	{
                        ...
                        expr *t = newexpr(newtable_e);
                          t->sym = newtemp();
                          printf("DOse mot to onoma\n %s",t->sym->name);
                          emit(tablecreate, NULL, NULL, t,yylineno,0);
                         // printf
                          int i=0;
                            for ( i = 0; $2;   $2 = $2->next){
                              printf("type:%s\n",expr_t_to_string($2->type));
                              emit(tablesetelem, newexpr_constnum(i++), $2->next,t,yylineno,0);
              ...
              
              {
                ...
                emit(tablesetelem, $2, $2->index,t,yylineno,0);
                // $$ = t;
               }
              $$ = t;
              }
...
block:        LEFT_CURLY_BRACKET{++current_scope; printf("LEft curly  bracket\n");} RIGHT_CURLY_BRACKET{Hide(symbolTable, current_scope--); printf("{ Statement }\n");$$=( stmt_t*)malloc(sizeof( stmt_t));  printf("Right curly bracket\n");
              ...
              | LEFT_CURLY_BRACKET{++current_scope ; } stmts RIGHT_CURLY_BRACKET {Hide(symbolTable, current_scope--);printf("Right curly bracket\n");$$=$3;} 
              ;
      ...
      SymbolTableEntry *tmp=  look_up_function_with_scope($1->sym->name,symbolTable,current_scope);
         ...
         tmp->value.funcVal->iadress=nextquadlabel();
             ...
             //  $$ = newexpr(var_e);
              ...
              ;   
funcblockstart:{printf("fncstrt\n"); loopcount=0;}//push(&loopcounterstack, loopcount); 
funcblockend:{printf("fncend\n");}
